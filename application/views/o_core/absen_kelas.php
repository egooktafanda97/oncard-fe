...
<div class="page-wrapper">
			<div class="page-content">
                ...
                <input type="date" id="selectedDate" class="form-control" style="width:500px;">
                <button type="button" class="btn btn-sm btn-primary ms-3" style="width:200px!important;" onclick="loadJadwal()">Load Jadwal</button>

                </div> -->

                    <div class="col-12" id="summaryDiv">
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                getJadwalPelajaran(moment(new Date()).format('YYYY-MM-DD'))
        ...
        function loadJadwal() {
            ...
            if (date) {
                getJadwalPelajaran(date);
            } else {
                alert('Please select a date');
            }
        }

        function getJadwalPelajaran(date) {
        ...
        function renderJadwalPelajaran(data, filterStatus = "ALL") {
            ...
            data.forEach(item => {
                ...
                const startDate = item.semester.tanggal_mulai;
                const endDate = item.semester.tanggal_selesai;

                if (todayDate < startDate || todayDate > endDate) {
                    return; // Skip if outside academic period
                }
                ...
                const absensiGuru = item.absensi?.find(absen => absen.user_type === 'GURU');

                if (!absensiGuru) {
                    countBelumAbsen++;
                }

                const guruName = absensiGuru?.user_absensi?.name || 'Belum Absen';
                const waktuAbsen = absensiGuru?.jam_absen || '-';
                const keteranganAbsen = absensiGuru?.keterangan_absen?.name || '-';
                ...
                Object.keys(groupedData[className]).forEach(subClassName => {
                    ...
                    contentHtml += `<div class="sub-class-group my-3">
                        ...
                        groupedData[className][subClassName].forEach(item => {
                            const hasAbsen = item.waktuAbsen !== '-';
                            const isPassed = item.statusbody?.toLowerCase().includes('passed');
                            let bgColor = '#fff';

                            if (isPassed && hasAbsen) {
                                bgColor = '#e7fbda';
                            } else if (isPassed && !hasAbsen) {
                                bgColor = '#f4dbdb';
                            } else if (!isPassed && hasAbsen) {
                                bgColor = '#e7fbda';
                            } // else tetap #fff
                            ...
                            contentHtml += `
                                <div class="schedule-card" style="background:${bgColor}">
                                    ...
                                    <div style="padding:5px; background:#fff;display:block; border-radius:5px;">
                                    <div style="font-size:11px;"><strong>Waktu Absen:</strong> ${item.waktuAbsen}</div>
                                    <div style="font-size:11px;"><strong>Keterangan:</strong> ${item.keteranganAbsen}</div>
                                    </div>
                                    <hr/>
           ...
           // Langkah 1: Ambil semua item yang belum absen
            Object.keys(groupedData).forEach(className => {
                Object.keys(groupedData[className]).forEach(subClassName => {
                    groupedData[className][subClassName].forEach(item => {
                        if (item.waktuAbsen === '-') {
            ...
            // Langkah 2: Group by nama guru
            const guruBelumAbsenMap = {};
            belumAbsenList.forEach(item => {
                const guruName = item.guru.name;
                if (!guruBelumAbsenMap[guruName]) {
                    guruBelumAbsenMap[guruName] = [];
                }
                guruBelumAbsenMap[guruName].push(item.matapelajaran.name+" ("+item.sub_kelas.name+")");
            });

            // Langkah 3: Hitung total mata pelajaran yang belum absen
            countBelumAbsen = belumAbsenList.length;

            // Langkah 4: Bangun HTML ringkasan
            let summaryHtml = `
                <div style="padding: 1rem; background-color: #ffe3e3; border-radius: 8px; margin-bottom: 1rem;">
                    <strong>${countBelumAbsen}</strong> mata pelajaran belum memiliki absensi guru.
                    <ul style="margin-top: 0.5rem; padding-left: 1rem;">
            `;

            Object.keys(guruBelumAbsenMap).forEach(guruName => {
                summaryHtml += `<li><strong>${guruName}</strong>: `;

                guruBelumAbsenMap[guruName].forEach(mp => {
                    summaryHtml += `<span style="display:inline-block; background-color:#ab908f; color:#f2f2f2; padding:2px 6px; border-radius:6px; font-size:11px; margin-right:4px; margin-top:2px;">${mp}</span>`;
                });
