...
<div class="page-wrapper">
				...
				<div class="modal-dialog modal-sm modal-dialog-centered">
					<div class="modal-content">
						...
						<div class="modal-body addmodify">
							<div class="row g-3">
								<div class="col-md-12">
									<label for="setTingkat" style="display:block;" class="form-label">Tingkat / sederajat</label>
        ...
        <script type="text/javascript">
			...
			$(document).ready(function () {
				...
				table = $('#example').DataTable( {
					...
					columns: [
						...
						{ data: 'name', title: 'Kelas' },          // Maps to 'age' field
						...
						{ 
							...
							render: function (data, type, row) {
								return `
									...
									<button class="btn btn-danger ms-4 btn-sm" style="font-size:11px;" onclick="openModalDeleteNew('${row.id}','kelas')"><i class="bx bx-trash" style="font-size:1.6em;"></i> Delete</button>
								`;
							}
						}
					],
			...
			function showDetails(str){
				...
				const save2 = async () => {
					const posts2 = await axios.get('<?= api_url_core(); ?>api/master/kelas/'+str, {
						headers: {
							'Authorization': 'Bearer ' + localStorage.getItem('_token')
						}
					}).catch((err) => {
					...
					if (posts2.status == 200) {
						...
						tableColumn+=`
							...
							<small>Kode</small><br/>
								<h6 class="text-danger">${posts2.data.kode_kelas}</h6>
							<p>
			...
			function modalAddModify(mode){
				...
				if(mode!='add'){
					...
					const save2 = async () => {
						const posts2 = await axios.get('<?= api_url_core(); ?>api/master/kelas/'+mode, {
							headers: {
								'Authorization': 'Bearer ' + localStorage.getItem('_token')
							}
						}).catch((err) => {
