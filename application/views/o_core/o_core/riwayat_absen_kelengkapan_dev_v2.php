...
<div class="page-wrapper">
    ...
    render: function (data, type, row) {
        ...
        const completeCount = masterAddons.filter(addon => 
            checkedAddons.some(a => a.m_absensi_addons_id === addon.id)
        ).length;

        // Create tooltip content
        const tooltipContent = masterAddons.map(addon => {
            const isChecked = checkedAddons.some(
                a => a.m_absensi_addons_id === addon.id
            );
            const statusIcon = isChecked ? '✓' : '✗';
            const statusClass = isChecked ? 'text-success' : 'text-danger';
            
            return `<div class="${statusClass}">${statusIcon} ${addon.nama}</div>`;
        }).join('');
        ...
        return `
            <span class="d-inline-block" 
                ...
                <span class="badge ${completeCount === masterAddons.length ? 'bg-success' : 'bg-warning'}">
                    ${completeCount}/${masterAddons.length} Lengkap
                </span>
            </span>
        `;
    }
