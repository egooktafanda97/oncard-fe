...
<body>
    ...
    <script>
        ...
        // Convert to GMT+7 (Indonesia Western Time)
        const todayGMT7 = new Date(today.getTime() + (7 * 60 * 60 * 1000));
        const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
        document.getElementById('current-date').textContent = todayGMT7.toLocaleDateString('id-ID', options);
        // Your function with the logic to find students without attendance
        async function callAllDataToUpdate(lastSegment) {
            try {
                // Get today's date in YYYY-MM-DD format
                todayFormatted = todayGMT7.toISOString().split('T')[0];
                ...
                const res3 = await axios.get('<?= base_url(); ?>CPanel_Admin/showdata_kelengkapan_dataabsensi_from_3rd_party/'+lastSegment+"/"+todayFormatted);
                const getTagihan = res3.data.data;

                // Process the data to find students without attendance
                processAttendanceData(getDataOrtuOncard, getWalitoSiswaTuition, getTagihan, todayFormatted);
...
Ananda ${student.nama_lengkap} tidak melakukan absensi pada hari ini (${todayFormatted}).
...
Jika ada kekeliruan pada informasi ini, harap hubungi Admin Sekolah. Terimakasih.`;
                ...
                row.innerHTML = `
                    ...
                    <td>${hasAttended ? 'Siswa telah absen hari ini' : `*INFORMASI ABSEN HARI INI*
Ananda ${student.nama_lengkap} tidak melakukan absensi pada hari ini (${todayFormatted}).

Jika ada kekeliruan pada informasi ini, harap hubungi Admin Sekolah. Terimakasih.`}</td>
                `;
			...
			// sendAllPushNotificationsSequentially(); //STOPPED BY ADMIN
        }
