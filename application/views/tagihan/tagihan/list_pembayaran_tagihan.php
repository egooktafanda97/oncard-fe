...
<div class="page-wrapper">
		...
		<script type="text/javascript">
            ...
            function showData(params) {
                ...
                // Add filter controls to #filterside
                if ($('#filterside').children().length === 0) {
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <select id="filterMetode" class="form-select form-select-sm">
                                    ...
                                    <div class="col-md-6 mb-2">
                                        <label class="form-label">Multipayment</label>
                                        <select id="filterMultipayment" class="form-select form-select-sm">
                                            <option value="">Semua</option>
                                            <option value="yes">Multipayment</option>
                                            <option value="no">Bukan Multipayment</option>
                                        </select>
                                    </div>
                                    <div class="col-md-6 mb-2">
                                    ...
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <input type="text" id="filterRangeTanggal" class="form-control form-control-sm daterange">
                                    </div>
                                    <div class="col-md-6 mb-2">
                                        <label class="form-label">Nama Tagihan</label>
                                        <input type="text" id="filterNamaTagihan" class="form-control form-control-sm" placeholder="Cari nama tagihan...">
                                    </div>
                                    <div class="col-md-6 mb-2">
                                        <label class="form-label">Kelas</label>
                                        <select id="filterKelas" class="form-select form-select-sm"></select>
                                    </div>
                                    <div class="col-md-12">
                                        <button id="resetFilters" class="btn btn-sm btn-outline-secondary w-100">Reset Filter</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    `;
                    $('#filterside').html(filterControls);
                    
                    // Initialize date range picker if daterangepicker is available
                    if (typeof $.fn.daterangepicker !== 'undefined') {
                        $('#filterRangeTanggal').daterangepicker({
                            locale: {
                                format: 'YYYY-MM-DD',
                                separator: ' to ',
                                applyLabel: 'Pilih',
                ...
                const save2 = async () => {
                    ...
                    if (posts2?.status === 200) {
                        ...
                        window.invoiceDataStore = posts2.data;
                        ...
                        // Create an array to store all rows for DataTable
                        const dataTableRows = [];
                        ...
                        for (const invoice in grouped) {
                            ...
                            group.forEach((mapping, index) => {
                                ...
                                const invoiceData = {
                                    ...
                                    is_multipayment: rowspan > 1 ? 'yes' : 'no',
                                    payment_method: ['BILLER-IPAYMU', 'BILLER-BRKS'].some(biller => mapping.oncard_invoice.includes(biller)) ? 'TRANSFER' : 'CASH'
                                };
                                ...
                                const $row = $(`
                                    <tr data-invoice="${mapping.oncard_invoice}" 
                                        data-multipayment="${rowspan > 1 ? 'yes' : 'no'}" 
                                        data-status="${mapping.status_pembayaran}" 
                                        data-tanggal="${moment(mapping.created_at).format('YYYY-MM-DD')}" 
                                        data-nama-tagihan="${mapping.nama_tagihan}" 
                                        data-payment-method="${['BILLER-IPAYMU', 'BILLER-BRKS'].some(biller => mapping.oncard_invoice.includes(biller)) ? 'TRANSFER' : 'CASH'}">
                                        <td>
                                        ...
                                        <td><b>${mapping.siswa?.nama_sub_kelas ?? 'not set'}</b></td>
                                        ...
                                        <td style="text-transform:capitalize;">${moment(mapping.created_at).format('YYYY-MM-DD, HH:mm')}</td>
                                    </tr>
                                `);
                                
                                // Add to DataTable rows array
                                dataTableRows.push($row);
                            });
                        }

                        // Destroy existing DataTable if it exists
                        if ($.fn.DataTable.isDataTable('#tabelPrint')) {
                            $('#tabelPrint').DataTable().destroy();
                        }

                        dataTableInstance = $('#tabelPrint').DataTable({
                            responsive: true,
                            ordering: true,
                            searching: true,
                            ...
                            columns: [
                                ...
                                { title: "Status" },
                                { title: "Aksi" },
                                { title: "Created At", visible: false } // hidden kolom untuk sorting
                            ],
                            order: [[11, "desc"]] // urut pakai kolom ke-12 (created_at)
                        });
                        ...
                        // After DataTable is drawn
                        dataTableInstance.on('draw', function() {
                            const kelasSelect = $('#filterKelas');
                            if (kelasSelect.children('option').length <= 1) { // only has placeholder
                                const kelasValues = new Set();

                                dataTableInstance.rows().every(function() {
                                    const row = this.node();
                                    const kelasValue = $(row).find('td').eq(3).text().trim(); // 3rd index = "Kelas"
                                    if (kelasValue) {
                                        kelasValues.add(kelasValue);
                                    }
                                });

                                // Clear old options except first
                                kelasSelect.find('option:not(:first)').remove();

                                // Add unique kelas values
                                Array.from(kelasValues).sort().forEach(val => {
                                    kelasSelect.append(`<option value="${val}">${val}</option>`);
                                });
                            }
                        });

                        // Add event listener for Kelas filter
                        $('#filterKelas').on('change', function() {
                            const selectedKelas = $(this).val().toLowerCase();
                            
                            dataTableInstance.rows().every(function() {
                                const row = this.node();
                                const kelasValue = $(row).find('td').eq(3).text().toLowerCase();
                                if (!selectedKelas || kelasValue === selectedKelas) {
                                    $(row).show();
                                } else {
                                    $(row).hide();
                                }
                        ...
                        function applyFilters() {
                            const filterMetode = $('#filterMetode').val();
                            const filterMultipayment = $('#filterMultipayment').val();
                            const filterStatus = $('#filterStatus').val();
                            const filterNamaTagihan = $('#filterNamaTagihan').val().toLowerCase();
                            const filterRangeTanggal = $('#filterRangeTanggal').val();
                            ...
                            dataTableInstance.rows().every(function() {
                                ...
                                if (filterMetode) {
                                    const paymentMethod = $row.attr('data-payment-method');
                                    if (paymentMethod !== filterMetode) {
                                        showRow = false;
                                    }
                                }
                                
                                // Filter by multipayment
                                if (filterMultipayment) {
                                    const multipayment = $row.attr('data-multipayment');
                                    if (multipayment !== filterMultipayment) {
                                        showRow = false;
                                    }
                                }
                                ...
                                // Filter by nama tagihan
                                if (filterNamaTagihan) {
                                    const namaTagihan = $row.attr('data-nama-tagihan').toLowerCase();
                                    if (!namaTagihan.includes(filterNamaTagihan)) {
                                        showRow = false;
                                    }
                                }
                                
                                // Filter by date range
                                if (filterRangeTanggal && filterRangeTanggal.includes(' to ')) {
                                    const dates = filterRangeTanggal.split(' to ');
                                    const startDate = moment(dates[0]);
                                    const endDate = moment(dates[1]);
                                    const rowDate = moment($row.attr('data-tanggal'));
                        ...
                        $('#filterMetode, #filterMultipayment, #filterStatus').on('change', applyFilters);
                        $('#filterNamaTagihan').on('keyup', applyFilters);
                        $('#filterRangeTanggal').on('change', applyFilters);
                        
                        // Reset filters button
                        $('#resetFilters').on('click', function() {
                            $('#filterMetode, #filterMultipayment, #filterStatus').val('');
                            $('#filterNamaTagihan').val('');
                            $('#filterRangeTanggal').val('');
                            applyFilters();
                        });

                        // Add event listener to the invoice buttons
                        $('#tabelPrint').on('click', '.btn-issue-invoice', function() {
                            try {
                        ...
                        $('#filterTanggal').off('change').on('change', function() {
                            // Destroy the DataTable before reloading
                            if ($.fn.DataTable.isDataTable('#tabelPrint')) {
                                $('#tabelPrint').DataTable().destroy();
                            }
