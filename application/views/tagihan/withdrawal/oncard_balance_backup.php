...
<div class="page-wrapper">
					 ...
					 <div class="card radius-2" style="border-radius:25px;">
						<div class="card-body" >
							<div class="d-lg-flex align-items-center mb-1 gap-3 px-2 pt-2">
                                <h3>Fee Admin (QRION)</h3>
						    </div>
                            
							<div class="d-lg-flex align-items-center mb-4 gap-3 px-2 pb-2">
                            <font class="text-sm">Saldo dibawah merupakan nominal yang dimiliki oleh Pihak QRION sebagai penyedia layanan.</font>
						    </div>
                    ...
                    <div class="card radius-2" style="border-radius:25px;">
						<div class="card-body" >
							<div class="d-lg-flex align-items-center mb-4 gap-3 px-2 py-2">
                                <h3>
                                <i class='bx bx-file'></i>
                                Histori Biaya Layanan</h3>
						    </div>

                            <div class="row xxx">
                                <table id="example2" class="tabelsiswa table table-hover tabelproduk" style="width:100%;font-size:12px!important;">
								<thead class="table-light">
									<tr>
										<th>No.</th>
										<th>Tanggal</th>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer text-center">
                        <div class="col-12 mt-5">
                            ...
                            data-action="commit" onclick="procReqWD(this);"
                            style="margin-top:-15px!important" 
        ...
        <script type="text/javascript">
            ...
            function procReqWD(button){


                Toastify({
                    text: "Tombol ini dalam masa perbaikan!",
                    duration: 3000,
                    close: true,
                    gravity: "top",
                    position: "right",
				...
				const save2 = async () => {
					...
					if (posts2.status == 200) {
                        ...
                        posts2.data.data.map((mapping,i)=>{
                            ...
                            if(mapping.account_type=='billing' && mapping.account_level=='owner'){
            ...
            function showReport2(){
                ...
                table = $('#example2').DataTable( {
					...
					order: [[0, 'asc']], // Order by "created_at" column in descending order (most recent first)
					ajax: function (data, callback) {
						axios.get('<?= api_url(); ?>api/v1/rep/jurnal?account_number='+selected_accountNumber+'&mtype=payment', {
							headers: {
								'Authorization': 'Bearer ' + localStorage.getItem('_token_oncard'),
                                'paginate' : true
							}
					...
					columns: [
						...
						{ 
							...
							title: 'Nominal', 
							render: function (data, type, row) {
								let myx = row.credit_amount??0;
								return `<span class="kode-kunjungan text-dark text-right" style="font-weight: 900 ">
											Rp${formatRupiah(myx.toString())}
										</span>`;
							} 
						}
