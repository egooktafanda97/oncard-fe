...
<script>
	...
	async function callAllDataToUpdate() {
		try {
			// 1. Parent User
			const res1 = await axios.get('<?= base_url(); ?>CPanel_Admin/ShowDataFCMTokenAutoBot_1_wa');
			...
			groupedArray = Object.values(finalGroupedByToken).map(group => ({
				title: group.title,
				body: `*Pengingat Tagihan Pendidikan[BOT]*\nBerikut kami informasikan \n${group.body}> Silahkan abaikan pesan ini atau lakukan pengecekan data pada aplikasi Qrion Bpk/Ibu apabila telah melakukan pelunasan sebelumnya.\n> *Qrion - Transforming Education With Digital Solution.*`,
			...
			function sendPushNotification(payload) {
				return axios.post('<?= base_url(); ?>CPanel_Admin/SendPushNotificationAutoBotMonoWA', payload).then(response => {
					Toastify({
