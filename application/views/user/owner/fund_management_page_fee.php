...
<div class="page-wrapper">
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('<?=base_url(); ?>CPanel_Admin/getDataFundManagementFee');
    ...
    function renderSimpleAdminFeeTable(data) {
        ...
        <thead class="table-light">
            <tr>
                ...
                <th class="text-end">Aktual Admin Fee Disekolah</th>
        ...
        Object.entries(data).forEach(([instansiId, instansi], index) => {
            ...
            <tr class="institution-row" data-id="${instansiId}">
                ...
                <td>${instansi.code}</td>
                <td class="text-end fw-bold">${formatCurrency(instansi.totalAdminFee)}</td>
                <td class="text-end">${instansi.transactionCount}</td>
                <td class="text-end fw-bold text-primary">${formatCurrency(instansi.totalSaldoBillingFee)}</td>
                <td class="text-end fw-bold ${differenceClass}">
                    ${formatCurrency(Math.abs(difference))}
                    <br>
                    <small class="${differenceClass}">
                        <i class="bx ${differenceIcon}"></i> ${differenceText}
                    </small>
                </td>
            </tr>`;
        });
        ...
        const grandTotalAdminFee = Object.values(data).reduce((sum, instansi) => {
            return sum + instansi.totalAdminFee;
        }, 0);
        ...
        html += `
            <tr class="table-active fw-bold">
                ...
                <td class="text-end">${formatCurrency(grandTotalSaldoBillingFee)}</td>
                <td class="text-end ${grandDifferenceClass}">${formatCurrency(Math.abs(grandTotalDifference))}</td>
            </tr>
        </tbody>`;
    ...
    function renderTransactionTableBody(transactions) {
        ...
        sortedTransactions.forEach(txn => {
            const adminFee = parseInt(txn.admin_fee) || 0;
            ...
            html += `
                <tr>
                    ...
                    <td><small>${txn.invoice_number}</small></td>
                    <td><small>${txn.userPengguna}</small></td>
                    <td><span class="badge bg-light text-dark">${txn.payment_type || 'N/A'}</span></td>
                    <td><span class="badge bg-light text-dark">${txn.bill_type || 'N/A'}</span></td>
