...
<div class="page-wrapper">
			<div class="page-content">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="col-12" id="setheremonth">
                        </div>
                    </div>
                </div>
				...
				<div class="card">
					<div class="card-body">
						<div class="table-responsive contentx">
                            Pilih salah satu tanggal diatas terlebih dahulu.
						</div>
					</div>
				</div>
        ...
        <script type="text/javascript">
            ...
            function modalSetInvoice(str){
                ...
                let foot = `
                    ...
                    <div class="col-12">
                        <div class="alert alert-info"><b>STATUS</b> PEMBAYARAN PENDING!</div>
                    </div>
                    </div>

                    <div class="row mt-3 mb-3">
                        <div class="col-12 text-center">
                            <button type="button" class="btn btn-success" onclick="modalSetPin(${str});">Selesaikan Invoice</button>
                        </div>
                    </div>

                `;
                $('#modalSetInvoice .modal-body').html(specificItem.html+foot);
			...
			$(document).ready(function () {
                ...
                // Generate and show buttons for the last 6 months
                let monthsContainer = $('<div id="months-container" style="margin: 10px;"></div>');
                let btn = '';
                for (let i = 0; i < 6; i++) {
                    let monthMoment = moment().subtract(i, 'months');
                    let firstDate = monthMoment.startOf('month').format('YYYY-MM-DD');
                    let monthName = monthMoment.format('MMMM YYYY');

                    // Create button
                    btn += `<button class="month-btn btn btn-sm btn-outline-primary" style="margin: 5px; padding: 6px 12px;" data-date="${firstDate}">
                                    ${monthName}
                                </button>`;
                    
                }

                $('#setheremonth').html(btn);

                // Set current month as default
                let first_date_current_month = moment().startOf('month').format('YYYY-MM-DD');
                setShowDataBasedOnAgencyID(first_date_current_month);

                // On button click, inject the selected date
                $(document).on('click', '.month-btn', function () {
                    let selectedDate = $(this).data('date');
                    setShowDataBasedOnAgencyID(selectedDate);
                });
            });
				...
				const save2 = async () => {
					...
					if (posts2.status == 200) {
                        ...
                        // console.log(posts2.data.data);
                            ...
                            instansi_id_pool.push(mapping.idINSTANSI);

							num += 1;
                            let mxxx = mapping.jumlah_saldo_available_ipaymu??0;


                            instansi_json_pool.push({
            ...
            function getPendingPayment(){
				...
				const save2 = async () => {
					const posts2 = await axios.get('<?=base_url(); ?>CPanel_Admin/getAllPaymentGatewaysPendingPayment').catch((err) => {
						console.log(err.response);
					});
                        ...
                        if(posts2.data.data.length>0){
                            ...
                            <div class="card">
                                <div class="card-header p-3">
                                    <h5>Tagihan Menunggu Penyelesaian</h5>
                                </div>
                                <div class="card-body">
            ...
            function getDatePool(){
                ...
                var form_data = new FormData();
                form_data.append('firstdate', first_date_current_month);
                form_data.append('lastdate', last_date_current_month);
                form_data.append('instansi_id_pool', instansi_id_pool);
				
				const save2 = async () => {
					const posts2 = await axios.post('<?=base_url(); ?>CPanel_Admin/getDatePoolInCurrentMonth', form_data).catch((err) => {
						console.log(err.response);
					});
					...
					if (posts2.status == 200) {
                        ...
                        // console.log(posts2.data.data);
							...
							num += 1;
                            tableColumn +=`
								<button type="button" onclick="setShowData('${mapping.date}');" class="btn btn-sm ${mapping.total_payments>0 ? 'btn-primary':mapping.date==moment(Date.now()).format('YYYY-MM-DD')?'btn-danger':'btn-outline-dark'}">${moment(mapping.date).format('DD')}</button>
							`;
							});
							
                        $('#buttonContainer').html(tableColumn);
            ...
            function setShowDataBasedOnAgencyID(str){
            // console.log(str);
            ...
            const save2 = async () => {
                const posts2 = await axios.get('<?=base_url(); ?>CPanel_Admin/getDataAgensiAllIpaymuByMonth/'+str+'/<?=$idInstansi;?>').catch((err) => {
                    console.log(err.response);
                });
                ...
                if (posts2.status == 200) {
                    ...
                    // console.log(posts2.data.data);
                        ...
                        posts2.data.data.map((mapping,i)=>{
                            dm += (mapping.jumlah_saldo_available_ipaymu? parseInt(mapping.jumlah_saldo_available_ipaymu) : 0);

                            if (mapping.payment_gateway_ids) { // Check if payment_gateway_ids is defined
                                const idsArray = mapping.payment_gateway_ids.split(',').map(id => parseInt(id, 10));
                                group_payment_ids.push(...idsArray);
                            }
                        });

                        // console.log("totalnya",dm)

                        tableColumn = `<h6 style="font-weight:normal; margin:10px;color:#ccc; text-align:center;">${posts2.data.date} ...</h6>
                        
                        <div class="col-12"  id="inv_x" class="inv_ix">
                            <div class="invoice-box">
                                <table cellpadding="0" cellspacing="0">
                                    <tr class="top">
                                        <td colspan="2">
                                            <table>
                                                <tr>
                                                    <td class="title">
                                                        <img src="https://oncard.id/assets/png/icon.png"
                                                            style="width: 100%; max-width: 100px" />
                                                    </td>
                                                    <td>
                                                        Invoice #: ${invoiceID}<br />
                                                        Dibuat: ${moment(str).format('MMMM YYYY')}<br />
                                                        Perlu dibayar: ${moment(str).format('MMMM YYYY')}
                                                    </td>
                                                </tr>
                                            </table>
                                        </td>
                                    ...
                                    <tr class="information">
                                        <td colspan="2">
                                            <table>
                                                <tr>
                                                    <td>
                                                        iPaymu 3rd Party Payment Gateway<br />
                                                        Bali<br />
                                                        Indonesia, +62
                                                    </td>
                                                    <td>
                                    ...
                                    <tr class="details">
                                        <td>Settlement Pool Payment Gateway to Oncard.id</td>
                                        <td>Rp${formatRupiah(dm.toString())}</td>
                                    </tr>
                        ...
                        tableColumn +=`</table></div>
                        
                        </div>
                        <button type="button" class="d-block mt-4 mb-4 btn btn-lg btn-success" onclick="submitCreateInvoice();" style="margin:auto;">Create Invoice</button>
            ...
            function generateRandomStringWithTimestamp() {
                ...
                return `PRBLN_${randomString.toUpperCase()}_${timestamp}`;
            }
		</script>