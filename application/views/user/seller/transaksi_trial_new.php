...
<div class="page-wrapper">
		...
		<script type="text/javascript">
			...
			function commitTransaction(){
				...
				if(mode=='pembelian'){
                    if(tipe_kartu==1){
                        url = '<?= api_url(); ?>api/trx/v3';
                    }else {
                        url = '<?= api_url(); ?>api/trx/v2';
                    }
				}else {
				...
				const save = async (form_data) => {
					const posts = await axios.post(url, form_data, {
                        ...
                        if(err.response.status==500||err.response.status==402){
                            ...
                            if (err.response.data.message.includes('Deadlock')) {
                                Toastify({
                                    text: "Mohon maaf, klik kembali tombol Selesai.",
                                    duration: 3000,
                                    close: true,
                                    gravity: "top",
                                    position: "right",
