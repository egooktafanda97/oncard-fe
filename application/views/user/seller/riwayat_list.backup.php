...
<div class="page-wrapper">
		...
		<script type="text/javascript">
            ...
            let oldDateKeep = '';
            let endPointGetDataSiswa = '<?= api_url(); ?>api/v1/rep/jurnal?account_level=users&mtype=sell';
			...
			$(document).ready(function () {
                ...
                $(".date-range").flatpickr({
                    ...
                    onClose: function(selectedDates, dateStr, instance){
                        ...
                        if(dateStr.length==1){
                            urlSetFromFilter = '<?= api_url(); ?>api/v1/rep/jurnal?date_from='+dateStr[0]+'&date_to='+dateStr[0]+'&account_level=users&mtype=sell'; 
                        }else {
                            urlSetFromFilter = '<?= api_url(); ?>api/v1/rep/jurnal?date_from='+dateStr[0]+'&date_to='+dateStr[1]+'&account_level=users&mtype=sell';
                        }
                        showData(endPointGetDataSiswa);
                        oldDateKeep = dateStr;
                    }
			...
			function showData(){
                ...
                if(isFilterOn){
                    ...
                    url = '<?= api_url(); ?>api/v1/rep/jurnal?account_level=users&mtype=sell';
                }
				...
				const save2 = async () => {
					...
					if (posts2.status == 200) {
                            ...
                            let sisasaldo = parseInt(mapping.balance)-parseInt(mapping.credit_amount);

                            invoiceDataArray.push({
                                invoice: mapping.invoice,
                                date: mapping.created_at,
                                total_spending: mapping.credit_amount,
                                admin_fee: '0',
                                total_payment: mapping.credit_amount,
                                account_users: mapping.debit_account?.customers_name??'-',
                                account_seller: mapping.credit_account.customers_name??'-',
                                saldo_awal: mapping.balance.toString(),
                                sisa_saldo: sisasaldo.toString(),
                            });

							num += 1;
                            j++;

                            filterTabungan += parseInt(mapping.credit_amount);
							...
							tableColumn +=`
								<tr>
									...
									<td>${mapping?.debit_account?.customers_name??'-'}</td>
									<td class="text-success" style="font-weight:bold;">Rp${formatRupiah(mapping.credit_amount)}</td>
									<td>Rp${formatRupiah(mapping.credit_amount)}</td>
									<td class="text-end">
                    ...
                    <div class="invoice-card" id="divToPRINT">
                        ...
                        <div class="invoice-details">
                            ...
                            <tbody class="detailTabelInvoice">
                                ...
                                <tr class="calc-row text-muted">
                                <td colspan="2"><b>Saldo Awal</b></td>
                                <td style="color:black!important;">Rp${formatRupiah(x['saldo_awal'])}</td>
                                </tr>
                                <tr class="calc-row">
                                <td colspan="2"><b>Saldo Akhir</b></td>
                                <td style="color:black!important;font-size:1.4em;font-weight:bold;">Rp${formatRupiah(x['sisa_saldo'])}</td>
                                </tr>
            ...
            $(".inputFilterItem").bind("change", function() {
                ...
                let ursets = '';
                urlsets = '<?= api_url(); ?>api/v1/rep/jurnal?account_level=users&mtype=sell';
                let valueData = '';
